...
public class ControladorSubsidio implements ActionListener {
    ...
    float porSubMat, valSubOng,valPagSub, dinTotSub100, dinTotSub80, dinTotSub60, dinTotSub40, dinTotSub20, dinTotSub, valMatSem, proSem;
...
public void btnEliminar(){
  ...
  public void verBusqueda(ArrayList<estudiantes> vecBus){
    ...
    for (estudiantes mb : vecBus) {
        Object contenido[] = {mb.getIdeEst(), mb.getApeEst(), mb.getNomEst(), mb.getProSem(), f3.format(mb.getValMatSem())};
            modelo2.addRow(contenido);
    }
    desahabilitarbtn();
    showPane(verVec);
