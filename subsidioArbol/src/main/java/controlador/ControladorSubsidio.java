...
public class ControladorSubsidio {
 ...
 private void btnCalcular() {
    ...
    try {
        if (est.getIdeEst() == null || est.getIdeEst().isEmpty()) {
            JOptionPane.showMessageDialog(null, "No hay estudiante validado para calcular", 
        ...
        int porcentajeRedondeado = Math.round(porSubMat);
        switch (porcentajeRedondeado) {
            case 100:
                numEstSub100++;
        ...
        Object contenido[] = {
            ...
            est.getApeEst(),           
            est.getNomEst(),           
            f3.format(est.getValMatSem()), 
            f2.format(porSubMat / 100), 
            f3.format(valSubOng), 
            f3.format(valPagSub)
        };
        ...
        JOptionPane.showMessageDialog(null, "Error en el cálculo: " + e.getMessage(), 
                                    "Error", JOptionPane.ERROR_MESSAGE);
    }
}
